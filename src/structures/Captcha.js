        ...
        if (!verifyDone) {
          ...
          if (data.times >= 3) {
            let banembed = new Discord.MessageEmbed()
              ...
              .setTimestamp()
              .setDescription(`${client.emoji.tick} Bạn đã bị thêm vào danh sách đen của bot vì đã nghi vấn sử dụng tool để cày nấm!`)
            member.send({ embeds: [banembed] })
            await client.blacklist.blackListUser(client, member.id)
            let uembed = new Discord.MessageEmbed()
              .setColor(client.config.botcolor)
              .setTimestamp()
              .setDescription(`${client.emoji.tick} Đã thêm **${member.tag}** vào danh sách đen vì đã nghi vấn sử dụng tool để cày nấm!`)
            client.channels.cache
              .get("982266843794403389")
              .send({ embeds: [uembed] });
            data.times = 0;
            data.counts = 0;
            data.totalCounts = 0;
